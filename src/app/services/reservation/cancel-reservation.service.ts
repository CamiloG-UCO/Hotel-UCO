...
export class CancelReservationService {
  ...
  private getAuthHeaders(): HttpHeaders {
    ...
    getDummyBooking(): Observable<any> {
        return this.http.get<any>(`${this.apiUrl}/dummy`, { headers: this.getAuthHeaders() });
    }

    getBookingByRoom(userEmail: string, roomCode: string): Observable<any> {
        return this.http.get<any>(`${this.apiUrl}/client/${userEmail}/room/${roomCode}`, { headers: this.getAuthHeaders() });
    }

    cancelBooking(userEmail: string, roomCode: string): Observable<string> {
        return this.http.delete(`${this.apiUrl}/client/${userEmail}/room/${roomCode}`, { 
